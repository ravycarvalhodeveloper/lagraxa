...
export async function getDataHome() {
    try {
        const res = await fetch(`${process.env.NEXT_PUBLIC_API_URL}/objects/667ad1c0864193998e15e89c?read_key=${process.env.READ_KEY}&depth=1&props=slug,title,metadata`, {next: {revalidate: 120}})
...
export async function getItemBySlug(itemSlug: string) {
    ...
    try {
        ...
        const rest = await fetch(url, {next: {revalidate: 120}})
